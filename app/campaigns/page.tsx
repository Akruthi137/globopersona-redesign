  ...
  return (
    <main className="min-h-screen bg-[#FAFBFF] text-slate-900">
      ...
      <div className="lg:pl-64 flex flex-col min-h-screen">
        ...
        <header className="h-24 bg-white border-b border-slate-100 flex items-center justify-between px-6 lg:px-12 sticky top-0 z-20">
          <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tight flex items-center gap-2">
              Campaigns <Sparkles className="text-indigo-500" size={20} />
            </h1>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Strategic Marketing Hub</p>
          </div>
          
          <button 
            onClick={() => setIsModalOpen(true)}
            className="flex items-center gap-2 bg-slate-900 hover:bg-indigo-600 text-white px-6 py-3 rounded-2xl font-bold text-sm shadow-xl transition-all active:scale-95"
          >
            ...
            <div key={c.id} className="group bg-white rounded-[2.5rem] border border-slate-100 p-8 shadow-sm hover:shadow-2xl hover:shadow-indigo-50 transition-all duration-500">
              <div className="flex justify-between items-start mb-8">
                ...
                <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${
                  ...
                  c.status === 'Scheduled' ? 'bg-amber-50 text-amber-600' : 'bg-slate-50 text-slate-500'
                }`}>
                  {c.status}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                ...
                {c.status === 'Draft' ? (
                  <button 
                    ...
                    className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-xl text-xs font-black shadow-lg transition-all active:scale-95"
                  >
                    Launch
                  </button>
                ) : (
          ...
          <div className="bg-white w-full max-w-md rounded-[3rem] p-10 shadow-2xl animate-in zoom-in-95 duration-500">
            ...
            <form onSubmit={handleAddCampaign} className="space-y-8">
              <div className="space-y-2">
                ...
                <input required value={newCampaignName} onChange={(e) => setNewCampaignName(e.target.value)} className="w-full p-5 bg-slate-50 border-none rounded-2xl outline-none font-bold text-slate-800 text-lg focus:ring-2 focus:ring-indigo-500 transition-all" placeholder="E.g., Spring Promo..." />
              </div>
